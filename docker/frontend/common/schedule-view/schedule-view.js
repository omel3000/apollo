(function() {
  ...
  function setupControls() {
    ...
    if (grid) {
      ...
      grid.addEventListener('click', (event) => {
        ...
        const dayCard = event.target.closest('.schedule-mini-calendar__day');
  ...
  function renderCalendar() {
    ...
    const firstWeekdayOffset = getCalendarWeekday(new Date(state.currentYear, state.currentMonth, 1));
    ...
    for (let i = 0; i < firstWeekdayOffset; i += 1) {
      ...
      grid.appendChild(createFillerCell());
    ...
    const trailingCells = totalCells - (firstWeekdayOffset + daysInMonth);
  ...
  function createDayCard(dayNumber, iso, entries, filteredEntries) {
    ...
    button.className = 'schedule-mini-calendar__day';

    button.setAttribute('data-date', iso);

    const description = buildDayDescription(iso, entries.length, filteredEntries.length);
    ...
    if (filteredEntries.length > 0) {
      ...
      button.classList.add('schedule-mini-calendar__day--busy');

    } else if (entries.length > 0) {

      button.classList.add('schedule-mini-calendar__day--hidden');

    } else {

      button.classList.add('schedule-mini-calendar__day--empty');
    ...
    if (iso === state.selectedDate) {
      ...
      button.classList.add('schedule-mini-calendar__day--selected');
    ...
    if (iso === toIsoDate(new Date())) {
      ...
      button.classList.add('schedule-mini-calendar__day--today');
    ...
    number.className = 'schedule-mini-calendar__number';
    ...
    if (filteredEntries.length > 0) {
      ...
      badge.className = 'schedule-mini-calendar__badge';
      ...
      mutedBadge.className = 'schedule-mini-calendar__badge schedule-mini-calendar__badge--muted';

      mutedBadge.textContent = entries.length;
  ...
  function highlightSelectedDay() {
    ...
    grid.querySelectorAll('.schedule-mini-calendar__day').forEach(card => {

      if (card.getAttribute('data-date') === state.selectedDate) {

        card.classList.add('schedule-mini-calendar__day--selected');

      } else {

        card.classList.remove('schedule-mini-calendar__day--selected');
  ...
  function renderWeekdayHeader() {
    ...
    const header = document.getElementById('scheduleCalendarWeekdays');
  ...
  function createFillerCell() {
    ...
    filler.className = 'schedule-mini-calendar__day schedule-mini-calendar__day--filler';
  ...
  function buildDayDescription(iso, totalEntries, visibleEntries) {

    if (totalEntries === 0) {

      return `${weekdayLabel(iso)} · Brak grafiku`;

    }

    if (visibleEntries === 0) {

      return `${weekdayLabel(iso)} · Wpisy ukryte przez filtry (${totalEntries})`;

    }

    return `${weekdayLabel(iso)} · Widoczne ${visibleEntries} z ${totalEntries} wpisów`;
